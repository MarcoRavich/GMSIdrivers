...
1. Legal & scope (first step)
   Confirm explicit legal permission to reverse-engineer the abandoned driver. Record license, EULA, and authorization. Stop if prohibited.

2. Containment and lab specifics (Win9x constraints)
   One host for analysis. Two VMs minimum:
   • Win95/98 VM (target). Use OSR2 images if device-support matters. Keep multiple snapshots.
   • Linux analysis VM for tooling (Ghidra, rizin, scripts).
   Prefer emulators (Bochs or QEMU) for dynamic tracing because they can intercept port I/O and MMIO. Host-only networking. No outbound uploads without review. Keep original package read-only and hashed.

3. Artifact acquisition and catalog
   Collect every file in the driver package: EXE, CAB, INF, SYS, VxD, VXD, DLL, COM, installers, README, firmware blobs. Compute sha256 for each. Extract installers with 7z / cabextract / UniExtract. Save extracted tree.

4. Identify format and driver model (critical)
...
5. Static triage focused on VxD/NE specifics
   • Extract strings across all binaries (strings -a -n 8). Look for port addresses, registry keys, device IDs.
   • Use Ghidra and rizin/radare2 for NE/VxD support; configure segmentation model for 16-bit segmented x86. Load the binary as NE/16-bit and check entry segment:offset.
...
6. Disassembly / decompilation for 16-bit code
   • Use at least two engines: Ghidra (with 16-bit project settings) plus rizin/retdec for cross-checks. IDA (if available) handles VxD well but license may restrict use.
...
7. Reconstruct initialization and resource negotiation
...
9. Prepare emulator-based dynamic tracing (preferred for Win9x)
...
10. Alternatives if emulator-only is insufficient
    • SoftICE for Win9x historically allowed kernel breakpoints. It is legacy, may be hard to source, and legally/ethically sensitive. Treat SoftICE as a last resort and verify licensing.
...
11. Dynamic tests to run in emulator
...
12. Reconstruct data structures and IOCTLs
...
13. Build local test harnesses and simulators
...
14. Clean-room reimplementation approach (if distribution needed)
...
15. Documentation and deliverables (Win9x-tailored)
...
16. Practical limitations and fallback guidance
...
17. Validation exit criteria for Win9x
...
18. Recommended short checklist to start immediately
